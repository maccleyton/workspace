...
class PaginationSystem {
	...
	createNewPage() {
		...
		header.textContent = `Página ${this.pageCounter}`;
	...
	addElementToPage(element, isHeading = false) {
		...
		let elementHeight = this.calculateElementHeight(element);

		// 💡 Se for um heading, prever o próximo bloco para evitar que o título fique isolado
		if (isHeading) {
			const next = element.nextElementSibling;
			if (next && /^(P|UL|OL|TABLE|PRE)$/.test(next.tagName)) {
				const nextHeight = this.calculateElementHeight(next);
				elementHeight += nextHeight * 0.9; // preve 90% da altura do bloco seguinte
			}
		}

		// 🔍 Decide se precisa criar nova página
		if (this.currentPage.height + elementHeight > this.contentHeight && this.currentPage.height > 50) {
			this.createNewPage();
		}

		// 🧩 Adiciona o elemento atual na página
...
function applyAllSettings() {
	...
	document.getElementById('zoomRange').textContent = settings.zoomLevel;
...
function setupSettingsPanel() {
	...
	document.getElementById('zoomRange').value = parseInt(settings.zoomLevel);
...
setTimeout(() => {
	document.querySelectorAll('.page-content').forEach(content => {
		// 💡 Protege valores monetários (R$ / US$) antes do KaTeX
		content.innerHTML = content.innerHTML.replace(
			/(\bR|\bUS)\$(\s?\d[\d\.,]*)/g,
			(m) => m.replace('$', '__CURRENCY__')
		);

		// Renderiza fórmulas reais
...
function renderMarkdown() {
	...
	// Configura o Marked sem highlight interno
	marked.setOptions({
		breaks: true,
		gfm: true
	});
	...
	try {
		...
		// 🔧 Corrige markup inconsistente do Marked
		htmlContent = htmlContent
			// remove <p> envolvendo listas
			.replace(/<\/h\d>\s*<p>\s*<ul>/g, match => match.replace('<p>', ''))
			.replace(/<\/ul>\s*<\/p>/g, '</ul>')
			// remove <p> dentro de <li>
			.replace(/<li>\s*<p>/g, '<li>')
			.replace(/<\/p>\s*<\/li>/g, '</li>')
			// remove <p> antes de <ol> também (listas numeradas)
			.replace(/<\/h\d>\s*<p>\s*<ol>/g, match => match.replace('<p>', ''))
			.replace(/<\/ol>\s*<\/p>/g, '</ol>');
		
		htmlContent = postprocessHTML(htmlContent);

		// Adiciona classes de linguagem ao código
		const tempDiv = document.createElement('div');
		tempDiv.innerHTML = htmlContent;
		
		// 🔧 Remove <p> solitário dentro de <li> (causa o vão maior no 1º item)
		tempDiv.querySelectorAll('li > p:first-child:last-child').forEach(p => {
		  const li = p.parentElement;
		  while (p.firstChild) li.insertBefore(p.firstChild, p); // move o conteúdo pra fora
		  p.remove();
		});
		...
		tempDiv.querySelectorAll('pre code').forEach((block) => {
			...
			if (!classList.includes('language-')) {
				block.classList.add('language-clike'); // fallback genérico
			}
		});

		// Renderiza nas páginas
		distributeContentToPages(tempDiv.innerHTML);

		// Realça código com Prism.js
		setTimeout(() => {
			Prism.highlightAllUnder(document.getElementById('pagesContainer'));
		}, 100);

	} catch (error) {
		console.error('Erro ao renderizar Markdown:', error);
		document.getElementById('pagesContainer').innerHTML =
			'<div class="a4-page"><div class="page-content"><p>Erro ao renderizar o conteúdo. Verifique o formato do Markdown.</p></div></div>';
	}
}
...
function preprocessMarkdown(text) {
	...
	text = text.replace(codeBlockRegex, m => m.replace(/\|/g, 'PIPE_PROTECTED'));

	const inlineCodeRegex = /`[^`\n]+?`/g;
	text = text.replace(inlineCodeRegex, m => m.replace(/\|/g, 'PIPE_PROTECTED'));

	// 💰 Protege valores monetários (R$, US$, etc.)
	text = text.replace(/(\bR|\bUS)\$(\s?\d[\d\.,]*)/g, (match) => {
		return match.replace(/\$/g, '&#36;'); // entidade HTML segura
	});
...
function applySettings() {
	...
	settings.zoomLevel = document.getElementById('zoomRange').value + '%';
